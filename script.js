...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showSlide(slideNumber) {
    ...
    const selectedSlide = document.querySelector(
      `.swiper-container[data-slide="${slideNumber}"]`
    );
...
document.querySelector("#submit").addEventListener("click", () => {
  ...
  if (
    ...
    const message = `Saya memberikan kritik atau saran \nName: ${nama}\nTelp: ${telp}\norang: ${person}\ntgl: ${tgl}\njam : ${jam}\npesan: ${pesan}`;
    const whatsappLink = `https://wa.me/+6282299901174?text=${encodeURIComponent(
      message
    )}`;
